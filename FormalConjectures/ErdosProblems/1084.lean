...
/--
English version:  The maximal number of pairs of points which are distance 1 apart that a set of `n` 1-separated
points in `ℝ^d` make. -/
...
/-- English version: It is easy to check that $f_2(n) < 3n$. -/@[category research solved, AMS 52]
theorem erdos_1084_easy_upper_d2 (hn : n ≠ 0) : f 2 n < 3 * n := by
  sorry

/-- English version: Erdős showed that there is some constant $c > 0$ such that $f_2(n) < 3n - c n^{1/2}$. -/@[category research solved, AMS 52]
theorem erdos_1084_upper_d2 : ∃ c > (0 : ℝ), ∀ n, f 2 n < 3 * n - c * sqrt n := by
  sorry

/--
English version:  Erdős conjectured that the triangular lattice is best possible in 2D, in particular that
$f_2(3n^2 + 3n + 1) < 9n^2 + 6n$. -/@[category research open, AMS 52]
theorem erdos_1084_triangular_optimal_d2 : f 2 (3 * n ^ 2 + 3 * n + 1) = 9 * n ^ 2 + 6 * n := by
  sorry

/--
English version:  Erdős claims the existence of two constants $c_1, c_2 > 0$
such that $6n - c_1 n^{2/3} ≤ f_3(n) \le 6n - c_2 n^{2/3}$. -/@[category research solved, AMS 52]
